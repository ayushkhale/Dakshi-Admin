...
const SignUp = () => {
  ...
  useEffect(() => {
    ...
    if (isLoggedIn) navigate('/dashboard'); // Redirect if already logged in
  }, [navigate]);

  // Get the email passed from the previous component (Signup)
  const location = useLocation();
  const email = location.state?.email || '';  // Assuming email is passed via state.

  useEffect(() => {
    // Trigger email OTP sending when component mounts
    if (email) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send OTP, username, email, and password in a single request to the backend for registration
      const response = await axios.post(
        'https://admin.dakshifoundation.in/v1/sign-up/verification',
